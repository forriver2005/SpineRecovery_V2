...
namespace UniVRM10
{
    public class MigrationTests
    {
        static string AliciaPath
        {
            get
            {
                return Path.GetFullPath(Application.dataPath + "/Tests/Replay/Fixtures/UniVRM/Alicia_vrm-0.51/AliciaSolid_vrm-0.51.vrm.bytes")
                    .Replace("\\", "/");
            }
        }
        ...
        public void MigrateMaterials()
        {
            ...
            var model106 = File.ReadAllBytes(Path.Combine(Application.dataPath, "Tests/Replay/Fixtures/UniVRM/Materials/EmissionMigration_v0.106.0.vrm.bytes"));
            // NOTE: Standard Shader の emission の値が linear value に変換されて emissiveFactor に出力される v0.107.0 のファイル
            var model107 = File.ReadAllBytes(Path.Combine(Application.dataPath, "Tests/Replay/Fixtures/UniVRM/Materials/EmissionMigration_v0.107.0.vrm.bytes"));
